import './LegalNotices.scss';
function LegalNotices() {
  return (
    <article className="LegalNotices">
      <h3 className="LegalNotices-title">Mention légales:</h3>
      <h5 className="LegalNotices-date">En vigueur au 19/07/2024</h5>
      <p className="LegalNotices-preface">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque
        vestibulum ex in nibh hendrerit sodales. Integer augue augue, maximus et
        mollis ac, commodo nec lectus. Sed eget pellentesque turpis, semper
        pharetra ante. Cras iaculis dui in dui elementum, nec fringilla felis
        euismod. Integer nec dolor sem. Aliquam erat volutpat. Pellentesque
        mollis condimentum enim, in scelerisque nulla lobortis non.
      </p>
      <div className="LegalNotices-content" />
      <h3>ARTICLE 1 - L'EDITEUR</h3>
      <p className="LegalNotices-editor">
        Nam lacinia nisi at augue dictum cursus. Vestibulum non vestibulum leo.
        Pellentesque auctor consectetur eros, quis aliquam erat. Aliquam erat
        volutpat. Pellentesque at velit auctor, vulputate orci in, pellentesque
        lorem.
      </p>
      <h3>ARTICLE 2 - L'HEBERGEUR</h3>
      <p className="LegalNotices-host">
        Nam lacinia nisi at augue dictum cursus. Vestibulum non vestibulum leo.
        Pellentesque auctor consectetur eros, quis aliquam erat. Aliquam erat
        volutpat. Pellentesque at velit auctor, vulputate orci in, pellentesque
        lorem.
      </p>
      <h3>ARTICLE 3 - ACCES AU SITE</h3>
      <p className="LegalNotices-websiteAccess">
        Nam lacinia nisi at augue dictum cursus. Vestibulum non vestibulum leo.
        Pellentesque auctor consectetur eros, quis aliquam erat. Aliquam erat
        volutpat. Pellentesque at velit auctor, vulputate orci in, pellentesque
        lorem.
      </p>
      <h3>ARTICLE 4 - COLLECTE DES DONNEES</h3>
      <p className="LegalNotices-CollectData">
        Fusce venenatis elit et ante bibendum, et luctus magna dignissim.
        Curabitur maximus eleifend quam, imperdiet aliquam mi interdum id. Cras
        gravida odio sit amet pharetra tincidunt. Phasellus viverra, mauris ac
        consectetur consequat, lectus dolor pellentesque sapien, non viverra
        velit mi facilisis erat. Phasellus non quam non magna volutpat
        malesuada. Mauris quis lorem a odio venenatis luctus quis ut leo. Donec
        consequat, felis vitae dapibus semper, lacus ex congue felis, eu rhoncus
        eros mi nec ipsum. Duis fringilla quis neque fermentum aliquet. Integer
        posuere efficitur turpis at malesuada. Fusce consequat bibendum tortor,
        a pellentesque libero porta eget. Nunc hendrerit ex id turpis euismod
        convallis eget et dui.
      </p>
    </article>
  );
}
export default LegalNotices;
